...
__bool AZ100_IsCompress(void *pBuffer, __u32 Size)
{
    ...
    {
        printf("invalid input argument for uncompress check\n");
        return EPDK_FALSE;
    }
...
__u32 AZ100_GetUncompressSize(void *pBuffer, __u32 Size)
{
    ...
    if (pBuffer == NULL || Size < AZ100_HEADER_LEN)
    {
        printf("invalid input argument for uncompress size get\n");
        return 0;
    }
    ...
    {
        //valid az100 compressed data
        printf("invalid az100 compressed data for uncompress size get\n");
        return 0;
    }
...
__s32 AZ100_Uncompress(void *pSource, __u32 SourceSize, void *pDest, __u32 DestSize)
{
    ...
    {
        printf("invalid input argument for data uncompress\n");
        return 0;
    }
...
__s32 AZ100_Compress(void *pSource, __u32 SourceSize, void *pDest, __u32 *DestSize)
{
    __s32 result;

    result = AZ100_DataCompress((__u8 *)pDest + AZ100_HEADER_LEN, DestSize, (__u8 *)pSource, SourceSize);
    memccpy((void *)pDest, (void *)AZ100_MAGIC, 1, AZ100_MAGIC_LEN);
    memccpy(pDest + AZ100_MAGIC_LEN, &SourceSize, 1, AZ100_MAGIC_LEN);
    return result;
}
